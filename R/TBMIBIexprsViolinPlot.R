...
# expression data. It asinh transforms the data and then visualizes violin plots of expression 
# values across the cell types. 
...
##..Subset just the mycobacterial samples and mono, mac, DC..##
...
IDO<-ggplot(data_myeloid, aes(x=fct_reorder(cell_type,IDO,.fun=median,.desc=TRUE), y=IDO, fill = cell_type)) +
  geom_violin(trim=FALSE, draw_quantiles = 0.5, position = position_dodge(width = 0.4)) +
