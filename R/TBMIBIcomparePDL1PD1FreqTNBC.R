...
dataAll_CSscaled<-read.csv("allsamples_dataCS3px_annotated.csv") #cell size normalized and untransformed for immune controls
dataGran_norm<-read.csv("granA_cellpheno_CS-asinh-norm_revised.csv") #cell size normalized, linearly + asinh scaled, 99th percentile scaled
dataTNBC_zscore<-read.csv("/Volumes/GoogleDrive/My Drive/Angelo Lab/MIBIProjects/Human_ATB_paper-cohort/all-fields/denoised/dataPerCell_3px/TNBC_shareCellData/cellData.csv") #TNBC data, cell size normalized, asinh, z-scored
...
all_myco <- c(granA_lung,granA_pleura,granA_LN,granA_endo) #all A granulomas
all_imm <- c(imm_tons,imm_spl,imm_plac) #all immune controls

# drop MAC from dataGran_norm
dataGran_norm<-droplevels(dataGran_norm[!dataGran_norm$SampleID %in% granA_MAC,])
...
dfmelt.ratio <- melt(all_samples_freq, id.vars = "tissue", measure.vars = c("Lag3.PDL1.Ratio", "PD1.PDL1.Ratio","Lag3.PD1.Ratio"))
